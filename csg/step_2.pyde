...
DIRECTIONAL_LIGHT_DIR = PVector(-1, 0.4, -1)
DIRECTIONAL_LIGHT_DIR.normalize()
...
def draw():
    ...
    for x in range(0, width, pixel_size):
        for y in range(0, height, pixel_size):
            ...
            if intersections:
                intersection_pt = ray_origin + ray_dir * intersections[0]
                pt_normal = (intersection_pt - sphere_center).normalize()
                intensity = max(0, pt_normal.dot(DIRECTIONAL_LIGHT_DIR))
                fill(constrain(224.0*intensity + AMBIENT_LIGHT, 0, 255))
